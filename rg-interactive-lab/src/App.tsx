...
export default function App(){
  ...
  return <div className="app"><a className="skip-link" href="#main-content">Ir para o conteúdo</a><header className="app-header"><a className="lab-brand" href="#/"><span>R<sup>μ</sup><sub>ν</sub></span><div><strong>RG Interactive Lab</strong><small>Capítulo 4 · Tensores</small></div></a><nav><a href="../capitulo-2/">Capítulo 2</a><a href={professor?"#/":"#/professor"}>{professor?"Modo aluno":"Professor Viewer"}</a><button aria-label={dark?"Ativar modo claro":"Ativar modo escuro"} onClick={()=>setDark(value=>!value)}>{dark?"☼":"◐"}</button>{session&&screen!=="welcome"&&!professor&&<button className="student-chip" onClick={goSummary} disabled={!session.locked}>{session.student.name.split(" ")[0]}<i>{session.locked?"relatório":"sessão local"}</i></button>}</nav></header>
